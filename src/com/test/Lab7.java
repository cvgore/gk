...
public class Lab7 extends JPanel implements GLEventListener {
    ...
    // C-port
    private void draw_pyramid_triangle3d(GL2 gl)
    {
    ...
    private void customPyramid(GL2 gl) {
        gl.glScaled(.1, .1,.1);

        gl.glBegin(gl.GL_TRIANGLES);
        {
            draw_pyramid_triangle3d(gl);
        }
        gl.glEnd();
        ...
        for (int i = 1; i < 8; ++i)
        {
            gl.glPushMatrix();
            {
                gl.glTranslatef(0.f, 0.f, 2.412f);
                gl.glRotatef(i * 45, 0.f, 1.f, 0.f);
                gl.glTranslatef(0.f, 0.f, -2.412f);
    ...
    private Texture textureFromPainting() {
        try {
            ...
            GL2 ctx = displayGL.getContext().getGL().getGL2();
            TextureData texData = TextureIO.newTextureData(ctx.getGLProfile(), fis, true, null);
            return TextureIO.newTexture(ctx, texData);
        } catch (IOException ex) {
            return null;
        }
    ...
    public JMenuBar getMenuBar() {
        ...
        makeMenuItem(objectMenu, "Pyramid (8)", objectListener, 7);
